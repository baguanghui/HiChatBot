...
class Preprocess():
    ...
    def main(self):
        ...
        q_a_df = pd.read_csv(os.path.join(self.dialog_dir,"question_answer.csv"), dtype=np.str)        
        Qs = q_a_df.question.values.astype(np.str)
        self.to_vec("Q", Qs)

        As = q_a_df.answer.values.astype(np.str)
        self.to_vec("A", As)
