...
def get_answer():
    ...
    print(question)
    response_json = get_response(question) # 获取问答
...
def get_response(question):
    ...
    if not retAnswer: #如果没有检索到，让交给模型预测回答
        retAnswer = seq.predict(question)#模型预测 
        print('模型预测:',retAnswer)
    else:
        print('问答检索:',retAnswer) #检索回答
...
def searchAnswer(question):
    ...
    '''
    results = text_index.search_answer(question) #问答检索
    if(len(results) > 0):
...
if __name__ == '__main__':    
    run(host='localhost', port=8080, debug=False, reloader=False) #开启服务